...
def _run_provision(
    ...
    **env_kwargs,
    ...
    cap = _provision_env_prog(
        monkeypatch, tmp_config_dir, tmp_state_dir, repo, **env_kwargs
    )
...
def test_warm_start_emits_skip_lines(
...
def test_cold_start_emits_building_then_check(
    monkeypatch, tmp_config_dir, tmp_state_dir, tmp_path
):
    repo = tmp_path / "backend"
    repo.mkdir()
...
def test_fatal_container_error_returns_4(
    monkeypatch, tmp_config_dir, tmp_state_dir, tmp_path
):
...
def test_worktree_warning_path_triggers_hold(
    monkeypatch, tmp_config_dir, tmp_state_dir, tmp_path
):
...
def test_host_only_project_skips_worktree_stage(
    monkeypatch, tmp_config_dir, tmp_state_dir, tmp_path
):
    ...
    rc = new.main(
        ["--provision", "--window-id", "@5", "--pane-id", "23", "--project", "tools"]
    )
    log = _read_log(tmp_state_dir)
    assert rc == 0
...
def test_fatal_worktree_error_returns_4(
    monkeypatch, tmp_config_dir, tmp_state_dir, tmp_path
):
...
def test_ssh_pump_timeout_warns_and_triggers_hold(
    ...
    monkeypatch.setattr(container, "current_name", lambda proj: "backend-c")
    rc, log, cap = _run_provision(
        monkeypatch, tmp_config_dir, tmp_state_dir, repo, pump_result="timed_out"
    )
    assert rc == 0
    assert "ssh pump" in log
...
def test_hooks_warning_also_logged(
    monkeypatch, tmp_config_dir, tmp_state_dir, tmp_path, tmux_agents_caplog
):
    ...
    assert "could not provision" in log
    assert any(
        "provisioning failed" in rec.message for rec in tmux_agents_caplog.records
    )
