...
def test_resolve_missing_worktree_creates(tmp_path, monkeypatch):
    ...
    assert calls == [["git", "-C", str(tmp_path), "worktree", "add", str(tmp_path / ".worktrees" / "feat-x"), "-B", "feat-x"]]

def test_resolve_in_container_uses_docker_exec(tmp_path, monkeypatch):
    calls = _stub_run(monkeypatch, [(0, "")])
    result = worktree.resolve(
        tmp_path, "feat-x",
        container="api-devcontainer", container_workdir="/work",
    )
    assert result == tmp_path / ".worktrees" / "feat-x"
    assert calls == [[
        "docker", "exec", "-u", "vscode", "api-devcontainer",
        "git", "-C", "/work", "worktree", "add",
        "/work/.worktrees/feat-x", "-B", "feat-x",
    ]]

def test_resolve_git_failure_raises(tmp_path, monkeypatch):
    _stub_run(monkeypatch, [(128, "fatal: branch 'feat-x' is already checked out at /other/path")])
...
def test_remove_in_container_uses_docker_exec(tmp_path, monkeypatch):
    ...
    assert calls == [[
        "docker", "exec", "-u", "vscode", "api-devcontainer",
        "git", "-C", "/work", "worktree", "remove",
        "/work/.worktrees/feat-x", "--force",
    ]]

def test_remove_when_absent_is_noop(tmp_path, monkeypatch):
    called = []
    monkeypatch.setattr(subprocess, "run", lambda *a, **k: called.append(a) or MagicMock(returncode=0, stdout=""))
...
def test_resolve_appends_commit_ish_when_base_resolves(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(worktree, "_resolve_base",
                        lambda *a, **kw: ("origin/main", []))
    calls = _stub_run(monkeypatch, [(0, "")])
    worktree.resolve(tmp_path, "feat-x")
    assert calls == [[
...
def test_resolve_skips_when_worktree_exists(monkeypatch, tmp_path):
    repo = tmp_path / "r"; (repo / ".worktrees" / "feat/x").mkdir(parents=True)
