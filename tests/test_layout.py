...
def test_layout_split_to_compact_kills_bottom_panes(monkeypatch, tmp_state_dir):
    ...
    monkeypatch.setattr(tmux, "list_panes", lambda w: [
...
def test_attach_overview_pane_splits_and_tags(monkeypatch):
    ...
    splits = []
    monkeypatch.setattr(tmux, "split_window", lambda w, *, percent, command: splits.append((w, percent, command)) or "%9")
    tagged = []
...
def test_layout_split_calls_attach_overview_for_each_non_ctrl(monkeypatch, tmp_state_dir):
    ...
    monkeypatch.setattr(overview, "attach_overview_pane",
                        lambda wid: attached.append(wid))
