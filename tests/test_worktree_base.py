...
@pytest.mark.parametrize(
    "routes,override,expected_commit_ish,warning_substr", SUCCESS_CASES
)
def test_resolve_base_success(
    tmp_path, monkeypatch, routes, override, expected_commit_ish, warning_substr
):
...
def test_resolve_base_auto_set_head_succeeds(tmp_path, monkeypatch):
    seq = iter(
        [
            ...
            MagicMock(
                returncode=0, stdout="git@github.com:foo/bar.git\n", stderr=""
            ),  # remote get-url
...
def test_resolve_base_auto_init_default_branch_local(tmp_path, monkeypatch):
    seq = iter(
        [
            ...
            MagicMock(
                returncode=0, stdout="git@github.com:foo/bar.git\n", stderr=""
            ),  # remote get-url (origin exists)
            MagicMock(
                returncode=1, stdout="", stderr="Could not resolve host"
            ),  # remote set-head fails
            MagicMock(
