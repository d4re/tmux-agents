...
def test_spawn_worker_uses_tmux_server_not_popen(monkeypatch):
    ...
    def _boom_popen(*a, **k):
        raise AssertionError("worker spawned via subprocess.Popen; must use tmux.run_shell_bg")
    ...
    assert shlex.split(calls[0]) == [
        "agent-new", "--provision", "--window-id", "@9",
        "--pane-id", "42", "--project", "myproj", "--branch", "feat/slash-ok",
    ]
...
def _provision_env(monkeypatch, tmp_config_dir, tmp_path, *, warn=False, fail_container=False):
    ...
    if fail_container:
        def boom(proj, *, up_cmd): raise container.ContainerError("docker down")
    ...
    monkeypatch.setattr(ssh_forward, "maybe_spawn_pump",
                        lambda c, u: ssh_forward.PumpResult("ready"))
...
def test_provision_success_respawns_into_claude(monkeypatch, tmp_config_dir, tmp_path):
    ...
    rc = new.main(["--provision", "--window-id", "@5", "--pane-id", "23", "--project", "api"])
    assert rc == 0
...
def test_provision_container_failure_shows_error_pane(monkeypatch, tmp_config_dir, tmp_path):
    from tmux_agents import windows as windows_mod
...
def test_new_container_with_branch_creates_worktree(monkeypatch, tmp_config_dir, tmp_path):
    ...
    windows_mod.write_mapping(windows_mod.WindowMapping(
        ...
        host_worktree=repo, pane_id="23", phase_hint="starting"))
    wt_calls = []
    def fake_resolve(repo_arg, branch, *, base_override=None, container=None,
...
def test_new_host_only_skips_container_up(monkeypatch, tmp_config_dir, tmp_path):
    ...
    windows_mod.write_mapping(windows_mod.WindowMapping(
        window_id="@5", project="scripts", branch=None,
        host_worktree=repo2, pane_id="23", phase_hint="starting"))
    monkeypatch.setattr(container_mod, "ensure_up",
    ...
    monkeypatch.setattr(startup_mod, "_respawn_with_retry",
                        lambda pid, cmd: cap.respawns.append((pid, cmd)))
    ...
    rc = new.main(["--provision", "--window-id", "@5", "--pane-id", "23", "--project", "scripts"])
    assert rc == 0
    assert cap.ensured == []
    assert len(cap.respawns) == 1
    _, cmd = cap.respawns[0]
...
def test_new_compact_layout_skips_split(agent_new_env, tmp_config_dir, tmp_path, tmp_state_dir):
...
def test_new_no_project_invokes_picker(agent_new_env, monkeypatch, tmp_config_dir, tmp_path):
    _write_config(tmp_config_dir, tmp_path)
    picked = []
    ...
    monkeypatch.setattr(pickers, "pick_or_create", lambda candidates, *, prompt, validator=None: None)
...
def test_new_picker_prompts_for_branch(agent_new_env, monkeypatch, tmp_config_dir, tmp_path):
    ...
    monkeypatch.setattr(pickers, "pick_or_create", lambda candidates, *, prompt, validator=None: "feat-x")
    monkeypatch.setattr(worktree, "list_existing", lambda r: [])
    monkeypatch.setattr(windows_mod, "live_branches_for", lambda p: set())
    rc = new.main([])
    assert rc == 0
...
def test_new_picker_cancelled_is_noop(agent_new_env, monkeypatch, tmp_config_dir, tmp_path):
    _write_config(tmp_config_dir, tmp_path)
...
def test_new_branch_prompt_cancelled_is_noop(agent_new_env, monkeypatch, tmp_config_dir, tmp_path):
    _write_config(tmp_config_dir, tmp_path)
    monkeypatch.setattr(pickers, "pick_one", lambda items, *, prompt: "api")
...
def test_new_devcontainer_resolved_name_flows_to_exec_cmd(monkeypatch, tmp_config_dir, tmp_path):
    ...
    monkeypatch.setattr(startup_mod, "_detach_stdio", lambda: None)
    windows_mod.write_mapping(windows_mod.WindowMapping(
        window_id="@5", project="webapp", branch=None,
        host_worktree=repo, pane_id="23", phase_hint="starting"))
    ...
    rc = new.main(["--provision", "--window-id", "@5", "--pane-id", "23", "--project", "webapp"])
    assert rc == 0
    assert len(cap.respawns) == 1
    _, cmd = cap.respawns[0]
    assert cmd == "docker exec -it brave_benz bash -lc 'cd /workspaces/webapp-gateway-service && claude'"


def test_new_creates_session_if_missing(monkeypatch, tmp_config_dir, tmp_path, tmp_state_dir):
    _write_config(tmp_config_dir, tmp_path)
    import subprocess
    from types import SimpleNamespace
    ...
    monkeypatch.setattr(_new_mod.subprocess, "Popen", lambda argv, **kw: SimpleNamespace(pid=0))
    ...
    monkeypatch.setattr(ssh_forward, "maybe_spawn_pump", lambda c, u: PumpResult("ready"))
...
def test_new_provisioning_is_idempotent(monkeypatch, tmp_config_dir, tmp_path):
    ...
    windows_mod.write_mapping(windows_mod.WindowMapping(
        window_id="@5", project="scripts", branch=None,
        host_worktree=repo2, pane_id="23", phase_hint="starting"))
    new.main(["--provision", "--window-id", "@5", "--pane-id", "23", "--project", "scripts"])
...
def _stub_ssh_pump_healthy_path(monkeypatch, *, has_python3=True, auth_sock="/tmp/agent.sock"):
    """Default ssh-pump bits: python3 present, auth sock set, no existing pumps."""
    monkeypatch.setattr(ssh_forward, "has_python3_in_container", lambda c, u: has_python3)
...
def test_new_skips_pump_spawn_when_responsive_pump_already_running(
    ...
    monkeypatch.setattr(ssh_forward, "kill_stale_pumps", lambda c: killed.append(c) or 0)
...
def test_new_kills_stale_pump_then_respawns_when_unresponsive(
    ...
    monkeypatch.setattr(
        ssh_forward, "maybe_spawn_pump",
        lambda c, u: results.append(ssh_forward.PumpResult("ready", killed_stale=2))
...
def test_new_skips_ssh_pump_when_forward_ssh_agent_false(monkeypatch, tmp_config_dir, tmp_path):
    ...
    windows_mod.write_mapping(windows_mod.WindowMapping(
        ...
        host_worktree=repo, pane_id="23", phase_hint="starting"))
    monkeypatch.setattr(container_mod, "current_name", lambda proj: None)
    monkeypatch.setattr(container_mod, "ensure_up", lambda proj, up_cmd: "api-devcontainer")
    pump_calls = []
    monkeypatch.setattr(ssh_forward, "maybe_spawn_pump",
                        lambda c, u: pump_calls.append((c, u)) or ssh_forward.PumpResult("ready"))
    monkeypatch.setattr(wt_mod, "resolve", lambda *a, **k: repo)
...
def test_new_skips_ssh_pump_when_python3_missing_with_warning(
    monkeypatch, tmp_config_dir, tmp_path, tmux_agents_caplog,
):
    ...
    with tmux_agents_caplog.at_level(logging.WARNING, logger="tmux_agents.ssh_forward"):
        new.main(["--provision", "--window-id", "@5", "--pane-id", "23", "--project", "api"])
...
def test_new_passes_empty_resume_args_to_substitute(monkeypatch, tmp_path, tmp_config_dir):
...
def test_interactive_branch_picker_sentinel_means_no_branch(
    ...
    assert captured["candidates"] == [
        pickers.NO_BRANCH_SENTINEL, "feat/foo", "fix-bar",
    ]
...
def test_interactive_branch_picker_open_marker_stripped(
    ...
    assert rc == 0
    assert captured["candidates"] == [
        pickers.NO_BRANCH_SENTINEL, "feat/foo  (open)", "fix-bar",
    ]
...
def test_new_inserts_after_last_project_sibling(
    ...
    _write_config(tmp_config_dir, tmp_path)
    windows_mod.write_mapping(windows_mod.WindowMapping(
        window_id="@4", project="api", branch=None,
        host_worktree=tmp_path, pane_id="23",
    ))
    windows_mod.write_mapping(windows_mod.WindowMapping(
        window_id="@6", project="scripts", branch=None,
        host_worktree=tmp_path, pane_id="24",
    ))
...
def test_new_interactive_creates_placeholder_and_spawns_worker(agent_new_env, tmp_config_dir, tmp_path):
...
def test_new_interactive_passes_branch_to_worker(agent_new_env, monkeypatch, tmp_config_dir, tmp_path):
    _write_config(tmp_config_dir, tmp_path)
    new.main(["api", "feat-x"])
    argv = agent_new_env.spawned[0]
...
def test_provision_unexpected_exception_lands_on_X(monkeypatch, tmp_config_dir, tmp_path):
    ...
    monkeypatch.setattr(worktree_mod, "resolve", lambda *a, **k: (_ for _ in ()).throw(RuntimeError("boom")))
    rc = new.main(["--provision", "--window-id", "@5", "--pane-id", "23",
                   "--project", "api", "--branch", "feat-x"])
    assert rc == 4
    assert len(cap.static_texts) == 1
...
def test_new_interactive_clears_stale_pane_state(agent_new_env, tmp_config_dir, tmp_path):
    ...
    rc = new.main(["api"])
    assert rc == 0
    assert not stale.exists(), "stale state file should have been unlinked before mapping write"
