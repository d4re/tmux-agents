...
def _active_windows():
    return [
        tmux.Window(
            id="@1", index=1, name="api:feat-x", active=False, state_code=state.RUNNING
        ),
        tmux.Window(
            id="@2",
            index=2,
        ...
        tmux.Window(
            id="@3", index=3, name="web:refactor", active=False, state_code=state.IDLE
        ),
    ]
...
def test_sleeping_count_renders_with_dot_suffix(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda s: [
            tmux.Window(
                id="@1", index=1, name="api:feat-x", state_code=f"{state.SLEEPING}2"
            )
        ],
    )
    out = _render_plain(overview.build_rows({}))
...
def test_background_count_renders_with_dot_suffix(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda s: [
            tmux.Window(
                id="@1", index=1, name="api:feat-x", state_code=f"{state.BACKGROUND}3"
            )
        ],
    )
    out = _render_plain(overview.build_rows({}))
...
def test_build_rows_carries_state_code_and_overlay_count(monkeypatch):
    monkeypatch.setattr(
        ...
        "list_windows",
        lambda s: [
            tmux.Window(id="@1", index=1, name="api:x", state_code=f"{state.SLEEPING}3")
        ],
    )
    rows = overview.build_rows({})
