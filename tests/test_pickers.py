...
@pytest.mark.parametrize("candidates,fzf_returncode,fzf_stdout,expected,validator", [
    ...
    (["weird name"], 0, "weird name\nweird name\n",   "weird name",  lambda s: " " not in s),
])
def test_pick_or_create_single_pass(
    monkeypatch, candidates, fzf_returncode, fzf_stdout, expected, validator,
):
    _stub_fzf(monkeypatch, fzf_returncode, fzf_stdout)
    assert pickers.pick_or_create(
        candidates, prompt="x> ", validator=validator,
    ) == expected
...
def test_pick_or_create_validator_rejects_typed_then_accepts(monkeypatch, capsys):
    ...
    monkeypatch.setattr(subprocess, "run",
                        lambda cmd, **kw: next(responses))
    result = pickers.pick_or_create(
        ["foo"], prompt="name> ", validator=lambda s: " " not in s,
    )
