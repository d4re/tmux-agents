...
from tmux_agents import config, container, logging_setup, overview, paths, phase, progress, provisioning, ssh_forward, startup, tmux, windows
...
def plan_entries(*, live_panes: dict[str, set[str]], projects: dict) -> list[Entry]:
    ...
    for f in snap.glob("*.json"):
        ...
        if project not in projects or not host_worktree.exists():
            continue
        e = Entry(
            window_id=f.stem, project=project, branch=d.get("branch"),
            host_worktree=host_worktree, pane_id=d.get("pane_id", ""),
...
def _mark_entry_failed(e: "Entry", ph: "Placeholder", reason: str) -> None:
    ...
    startup._write_pane_state(e.host_worktree, ph.pane_id.lstrip("%"), phase_value=phase.ERRORED)
...
def _pre_create_revive(e: Entry, live_panes: dict[str, set[str]]) -> "Placeholder | None":
    ...
    if len(survivors) == 1:
        ...
        if not overview_panes:
            logger.warning("%s: cannot revive — %d panes survive, none tagged "
                           "overview (%s)", e.window_id, len(survivors), survivors)
            return None
    ...
    new_full_pane_id = tmux.split_window(
        target, percent=75, command=startup.placeholder_command(e.window_id),
        before=True,
    )
    ...
    windows.write_mapping(windows.WindowMapping(
        window_id=e.window_id, project=e.project, branch=e.branch,
        host_worktree=e.host_worktree, pane_id=new_pane_stripped,
        claude_session_id=e.claude_session_id,
    ))
    startup._write_pane_state(e.host_worktree, new_pane_stripped, phase_value=phase.STARTING)
    logger.info("%s: revived -> pane=%s (split %s)",
                e.window_id, new_full_pane_id, target)
    return Placeholder(e, e.window_id, new_full_pane_id)


def pre_create_windows(plan: list[Entry],
                       live_panes: dict[str, set[str]]) -> dict[str, Placeholder]:
        ...
        try:
            ...
            if layout == "split":
                try:
                    ...
                    logger.warning("%s: overview-pane attach failed",
                                   e.window_id, exc_info=True)
            windows.write_mapping(windows.WindowMapping(
                window_id=new_wid, project=e.project, branch=e.branch,
                host_worktree=e.host_worktree, pane_id=pane_stripped,
                claude_session_id=e.claude_session_id,
            ))
            startup._write_pane_state(e.host_worktree, pane_stripped, phase_value=phase.STARTING)
            placeholders[new_wid] = Placeholder(e, new_wid, full_pane_id)
            logger.info("%s: pre-created -> %s pane=%s",
                        e.window_id, new_wid, full_pane_id)
        except Exception:
    ...
    if e.claude_session_id:
        ...
        if "{resume_args}" not in proj.exec_cmd:
            logger.warning(
                ...
                e.window_id, proj.name,
            )
    return proj.substitute(
        proj.exec_cmd, branch=e.branch,
        container_name=container_name, resume_args=resume_args,
    )


def _activate_project(project_name: str, entries: list[Entry],
                      projects: dict,
    ...
    try:
        for e in entries:
            ...
            files[e.window_id] = f
            reporters[e.window_id] = progress.Reporter(out=f, color=True,
            ...
            if proj.is_container:
                with multi.stage("container") as st:
                    ...
                    if existing:
                        ...
                        up_cmd = proj.substitute(proj.up_cmd, branch=None) if proj.up_cmd else None
        ...
        with resources.as_file(template_path) as template_file:
            for e in entries:
                ...
                if ph is None:
                    logger.warning("%s: no placeholder pane (pre_create skipped this entry)",
                                   e.window_id)
                    continue
                r = reporters[e.window_id]
                with r.stage("hooks") as st:
                    try:
                        provisioning.provision_settings(e.host_worktree,
                                                        template_path=template_file)
                    except Exception as ex:
                        st.warn(f"could not provision .claude/settings.local.json: "
                                f"{type(ex).__name__}: {ex}")
                        logger.warning("%s: provisioning failed (non-fatal)",
                                       e.window_id, exc_info=True)
                try:
                    cmd = _build_exec_cmd(proj, e, container_name)
                    startup._respawn_with_retry(ph.pane_id, cmd)
                    logger.info("%s: respawned pane=%s cmd_preview=%r",
                                e.window_id, ph.pane_id, cmd[:80])
                except Exception as ex:
...
def execute_plan(plan: list[Entry],
                 placeholders: dict[str, Placeholder],
                 projects: dict) -> None:
    ...
    with ThreadPoolExecutor(max_workers=4) as ex:
        futures: list[Future] = [
            ex.submit(_activate_project, name, list(entries), projects, by_entry_window, _fail)
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info("plan: %d entries; summary=%s", len(plan),
                [(e.window_id, e.project, e.branch, bool(e.claude_session_id), e.kind) for e in plan])
