...
def legacy_new_session_argv(conf: "Path") -> list[str]:
    ...
    return [*_TMUX, "-f", str(conf), "new-session", "-A",
            "-s", SESSION, "-n", CONTROL_WINDOW]
...
class Window:
    ...
    name: str
    active: bool = False
    state_code: str = ""  # @state_code option: display letter (+overlay), "" until first tick
...
def _run(args: list[str], *, check: bool = False) -> subprocess.CompletedProcess:
    ...
    if check and proc.returncode != 0:
        raise TmuxError(proc.returncode, proc.args, output=proc.stdout, stderr=proc.stderr)
    return proc
...
def list_windows(session: str) -> list[Window]:
    ...
    out = _run(["list-windows", "-t", session, "-F",
    ...
    for line in out.splitlines():
        ...
        windows.append(Window(id=wid, index=int(idx), name=name,
                              active=active == "1", state_code=code))
    return windows

def list_panes(window_id: str) -> list[Pane]:
    out = _run(["list-panes", "-t", window_id, "-F", "#{pane_id}\t#{pane_index}"]).stdout
    panes: list[Pane] = []
    for line in out.splitlines():
        if not line.strip():
            continue
...
def overview_pane_ids(window_id: str) -> list[str]:
    ...
    out = _run(["list-panes", "-t", window_id,
                "-F", "#{pane_id}\t#{@role}"]).stdout
    ids: list[str] = []
    for line in out.splitlines():
        if not line.strip():
            continue
...
def new_window(session: str, *, name: str, command: str,
               after_target: str | None = None) -> str:
...
def split_window(target: str, *, percent: int, command: str,
    ...
    args = ["split-window", "-v", "-d", "-l", f"{percent}%",
            "-P", "-F", "#{pane_id}", "-t", target]
    if before:
        args.insert(1, "-b")
    args.append(command)
    return _run(args, check=True).stdout.strip()
...
def window_pane_map(session: str) -> dict[str, set[str]]:
    ...
    r = _run(["list-panes", "-s", "-t", session,
              "-F", "#{window_id}\t#{pane_id}\t#{pane_dead}"],
             check=True)
...
def start_server_detached_with_session(
    *, conf: "Path", session: str, window_name: str,
) -> None:
    ...
    subprocess.run(
        [*_TMUX, "-f", str(conf), "new-session", "-d", "-s", session, "-n", window_name],
        check=True,
    )
